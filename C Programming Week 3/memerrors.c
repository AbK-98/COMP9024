...
void 
f1(void) { //dst did not have malloc 
    char *src = "f1";
...
f3(void) {
    ...
    char *dst = malloc(strlen(src) + 1);
    assert(dst != NULL);
    strcpy(dst, src); // src or dst was not used
    printf("%s\n", dst);
    free(dst);
}
...
f5(void) {
    ...
    assert(dst != NULL);
    strcpy(dst, src);
    printf("%s\n", dst); // need to print before free else its unintialised
    free(dst);
...
myprintf(char *string) {
    printf("%s\n", string);
    //free(string); No need for this free
}
...
f7(void) {
    ...
    strcpy(dst, src);
    printf("%s\n", dst);
    //free(src); No need for this free
    free(dst);
}
...
f8(void) {
    char *src = "f8";
    char *dst = malloc(strlen(src) + 1);
    assert(dst != NULL); // need to put check 
    strcpy(dst, src);
    printf("%s\n", dst);
    free(dst);
}
